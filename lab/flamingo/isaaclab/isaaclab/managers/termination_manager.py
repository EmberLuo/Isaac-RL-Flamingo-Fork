"""
Custom termination manager that patches the IsaacLab version to be compatible with 2.3
"""
    ...
    * **Terminated**: This signal is set to true if the environment has reached a terminal state defined by the
    ...
    def compute(self) -> torch.Tensor:
        ...
        for i, term_cfg in enumerate(self._term_cfgs):
            ...
            # Ensure value is a boolean tensor of shape (num_envs,)
            if torch.is_tensor(value):
                # If it's already a tensor, make sure it's the right shape and type
                if value.ndim == 0:  # scalar tensor
                    # Expand scalar tensor to match number of environments
                    value = value.expand(self.num_envs).clone()
                elif value.shape != (self.num_envs,):
                    # Handle mismatched shapes by broadcasting or reshaping appropriately
                    if value.numel() == 1:
                        # Single-value tensor, broadcast to all envs
                        value = value.expand(self.num_envs).clone()
                    elif value.shape[0] == self.num_envs:
                        # First dimension matches, ensure correct dtype
                        value = value.to(dtype=torch.bool)
                    else:
                        # Incompatible shape, default to all False
                        value = torch.zeros(self.num_envs, device=self.device, dtype=torch.bool)
                
                # Ensure the tensor is of boolean type
                value = value.to(dtype=torch.bool)
            elif isinstance(value, (bool, int, float)):
                # Convert scalar values to tensor of appropriate size
                value = torch.full((self.num_envs,), bool(value), device=self.device, dtype=torch.bool)
            else:
                # Handle other types (lists, arrays, etc.) or fallback
                try:
                    # Try to convert to tensor
                    value = torch.tensor(value, device=self.device, dtype=torch.bool)
                    
                    # Check shape and adjust if needed
                    if value.ndim == 0:  # scalar
                        ...
                        if value.numel() == 1:
                            ...
                            value = value.to(dtype=torch.bool)  # Ensure correct dtype
                        else:
                            # Incompatible, default to all False
                            value = torch.zeros(self.num_envs, device=self.device, dtype=torch.bool)
                except Exception:
                    # If conversion fails, default to all False
                    value = torch.zeros(self.num_envs, device=self.device, dtype=torch.bool)
